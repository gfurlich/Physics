#-------------------------------------------------------------------------------------#
#
#    File :                 fd_pedestal_weat_class_cnn.py
#    Author :               Greg Furlich
#    Date Created :         2019-03-26
#
#    Purpose: To Classifiy Weather of parts into clear, cloudy, and bad given the last frame in each part in a CNN
#
#    Execution :   python fd_pedestal_weat_class_cnn.py
#
#---# Start of Script #---#
...
def weather_class():
    ...
    train_gen = partLastFrameDataGenerator(in_files = train_files, labels = train_labels, batch_size = 64)
    val_gen = partLastFrameDataGenerator(in_files = val_files, labels = val_labels, batch_size = len(val_labels))
    all_gen = partLastFrameDataGenerator(in_files = files, labels = weat_df['part_weather_status'], batch_size = len(files), shuffle=False)
    ...
    model = keras.Sequential([
        ...
        # CPU #
        keras.layers.Conv2D(32, (4,4), input_shape=( nrows, ncols, 1), activation='relu', data_format='channels_last'),
        keras.layers.MaxPooling2D(pool_size=(2, 2)),
        keras.layers.Dropout(0.3),

        # Second Convolution Layer #
        # CPU #
        keras.layers.Conv2D(32, (4,4), activation='relu', data_format='channels_last'),
        keras.layers.MaxPooling2D(pool_size=(2, 2)),
        keras.layers.Dropout(0.3),
    ...
    print('Validation Accurracy of CNN Model : {0:.2f}% Loss:  {1:.2f}'.format( scores[1]*100,  scores[0]))
    ...
    plt.savefig(scratch_out+'weather_class_train_cnn_acc.png', bbox_inches='tight')
    plt.clf()
    ...
    y_pred = model.predict_generator(all_gen)
    y_classes = np.argmax(y_pred, axis=-1)
    y = np.array(weat_df['part_weather_status'])
    print(y.shape, y_classes.shape)
...
class partLastFrameDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, in_files):
        ...
        # CPU #
        X = np.empty((self.batch_size,)+ self.frame_dim + (1,) )
        ...
        for i, file in enumerate(in_files):
            ...
            last_frame = np.reshape(last_frame, last_frame.shape + (1,) )
