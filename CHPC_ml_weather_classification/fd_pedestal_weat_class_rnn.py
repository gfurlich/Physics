#-------------------------------------------------------------------------------------#
#
#    File :                 fd_pedestal_weat_class_rnn.py
#    Author :               Greg Furlich
...
#
#    Execution :   python fd_pedestal_weat_class_rnn.py
#
#---# Start of Script #---#
...
scratch_out = scratch+'RNN_out/'

# File Paths of Data for Generators #
file = scratch + 'RNN_Vectorized_Data/{0}_ped_fluct_vectorized.npy'
labels_hdf_file = scratch + 'RNN_DF/master_br_fd_ped_db_by_part_classified.csv'

# Out Models #
ml_weights_out = scratch_out + 'rnn_parameters.h5'
ml_model_out = scratch_out + 'rnn_model.json'
...
def weather_class():
    ...
    train_gen = partLastFrameDataGenerator(in_files = train_files, labels = train_labels, batch_size=16)
    val_gen = partLastFrameDataGenerator(in_files = val_files, labels = val_labels, batch_size = 16)
    ...
    model = keras.Sequential([
        ...
        # Input into GPU Acclerated LSTM Layer #
        keras.layers.TimeDistributed(keras.layers.Flatten(), input_shape=(t_dim, nrows, ncols)),

        # keras.layers.CuDNNLSTM(3072),
    ...
    print('Validation Accuracy of RNN Model : {0:.2f}% Loss:  {1:.2f}'.format( scores[1]*100,  scores[0]))

    ## Plotting model performance ##
    print('Plotting RNN Model Performance...')
    ...
    plt.savefig(scratch_out+'rnn_crossentropy.png', bbox_inches='tight')
    plt.clf()
    ...
    plt.legend()
    # plt.text(0.45, 0.5, 'Preliminary', horizontalalignment='center', verticalalignment='center', color='gray', fontsize=28, fontweight='bold', rotation=30, transform=ax.transAxes)
    plt.savefig(scratch_out+'rnn_accuracy.png', bbox_inches='tight')
    plt.clf()
    ...
    plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True, title='RNN Training Data Normalized Confusion Matrix')
    plt.savefig(scratch_out+'rnn_val_cfm_norm.png', bbox_inches='tight')
    plt.clf()

    plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=False, title='RNN Training Data Confusion Matrix')
    plt.savefig(scratch_out+'rnn_val_cfm.png', bbox_inches='tight')
    plt.clf()

    ## Save CNN Model ##
    print('Saving Trained RNN Model to {0}'.format(ml_model_out))
    print('Saving Trained RNN Weights to {0}'.format(ml_weights_out))
...
class partLastFrameDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, in_files):
        ...
        X = np.empty((self.batch_size,) + (self.t_dim,) + self.frame_dim )
        ...
        for i, file in enumerate(in_files):
            ...
            # Pad Array with Zeroed Arrays#
            n_frames = len(frames)
            n_add_frames = t_dim - n_frames
            pad_frames = np.zeros((n_add_frames, nrows, ncols))
            padded_frames = np.concatenate((pad_frames, frames), axis=0)

            # Flatten frame from 2D to 1D #
            # frames = np.array([padded_frames[i,:,:].flatten() for i in range(len(padded_frames))])
