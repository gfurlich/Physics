...
model_optimizer = keras.optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=1e-08, decay=0.0)
...
def weather_class():
    ...
    batch_size = 64
    train_gen = partFlattenFramesDataGenerator(in_files = train_files, labels = train_labels, batch_size = batch_size)
    val_gen = partFlattenFramesDataGenerator(in_files = val_files, labels = val_labels, batch_size = batch_size)

    ## Training Data ##
    val_X, val_y = partFlattenFramesDataGenerator(in_files = val_files, labels = val_labels, batch_size = len(val_files), shuffle=False).__getitem__(0)
    ...
    model = keras.Sequential([
        ...
        keras.layers.Dense(48, input_shape=(nrows*ncols,), activation='relu'),
    ...
    plt.savefig(scratch_out+'dnn_acc.png', bbox_inches='tight')
    plt.clf()
    ...
    plt.legend()
    # plt.text(0.45, 0.5, 'Preliminary', horizontalalignment='center', verticalalignment='center', color='gray', fontsize=28, fontweight='bold', rotation=30, transform=ax.transAxes)
    plt.savefig(scratch_out+'dnn_loss.png', bbox_inches='tight')
    plt.clf()
    ...
    plt.savefig(scratch_out+'dnn_cfm_val_norm.png', bbox_inches='tight')
    plt.clf()

    plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=False, title='DNN Validation Set Confusion Matrix')
    plt.savefig(scratch_out+'dnn_cfm_val.png', bbox_inches='tight')
    plt.clf()
